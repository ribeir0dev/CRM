...
<div class="modal fade" id="modalNovaTarefa" tabindex="-1" aria-labelledby="modalNovaTarefaLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form method="post" action="/actions/salvar_tarefa.php" id="form-tarefa-projeto">
...
<div class="modal fade" id="modalEditarTarefa" tabindex="-1" aria-labelledby="modalEditarTarefaLabel"
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form method="post" action="/actions/salvar_tarefa.php" id="form-editar-tarefa">
        <input type="hidden" name="projeto_id" value="<?= $projeto_id ?>">
...
<script>
  // Sincroniza select -> hidden 'coluna'
  document.addEventListener('DOMContentLoaded', function () {
    var colNew = document.getElementById('tarefaColunaSelect');
    var colNewH = document.getElementById('tarefaColuna');
    if (colNew && colNewH) {
      colNew.addEventListener('change', function () {
        colNewH.value = this.value;
      });
    }
    ...
    // Preencher modal de edição ao abrir
    ...
    if (modalEditar) {
      modalEditar.addEventListener('show.bs.modal', function (event) {
        ...
        document.getElementById('editTarefaTitulo').value = '';
        document.getElementById('editTarefaDescricao').value = '';
        document.getElementById('editTarefaDataEntrega').value = '';
        colEdit.value = 'backlog';
        colEditH.value = 'backlog';

        fetch('/actions/carregar_tarefa.php?id=' + encodeURIComponent(tarefaId))
          .then(function (r) { return r.ok ? r.json() : null; })
          .then(function (data) {
            if (!data) return;
            document.getElementById('editTarefaTitulo').value = data.titulo || '';
            document.getElementById('editTarefaDescricao').value = data.descricao || '';
            document.getElementById('editTarefaDataEntrega').value = data.data_entrega || '';
            colEdit.value = data.coluna || 'backlog';
            colEditH.value = data.coluna || 'backlog';
